# 09. Cache Busting and Invalidation
...
For the Deploy specifics, see [Chapter 10](./10-hq-extensions-and-cache.md). For output caching itself, see [Chapter 3](./03-website-output-caching.md).
...
- [Edge Cache in Front of the CDA](./05-edge-cache-in-front-of-the-cda.md) — the vendor-neutral pattern for the "CDN purge call" mentioned above, with Cloudflare, Azure API Management, and Azure Front Door specifics.
- [Chapter 10 - HQ Extensions and Cache](./10-hq-extensions-and-cache.md) — how Deploy deliberately changes this busting story.
- [Chapter 12 - Small Local Cache Example with Tags](./12-small-local-cache-example-with-tags.md) — the smallest possible fill-and-bust loop, in your own code.
...
[^04-core]: See [C7 in the appendix](./17-appendix-sources.md#c7-core-cache-types-and-refreshers) and [U6](./17-appendix-sources.md#u6-server-side-extensions-cache-docs).
[^04-refresher]: See [C7](./17-appendix-sources.md#c7-core-cache-types-and-refreshers) and [C6](./17-appendix-sources.md#c6-website-output-cache-implementation).
[^04-worked-trace]: See [C7](./17-appendix-sources.md#c7-core-cache-types-and-refreshers), [C6](./17-appendix-sources.md#c6-website-output-cache-implementation), and [C4](./17-appendix-sources.md#c4-umbracopublishedcachehybridcache-on-main). The GUID values in the trace are illustrative sample keys, not constants from Umbraco.
[^04-field-instructions]: See [F7 in the appendix](./17-appendix-sources.md#f7-distributed-cache-field-reports-v17).
[^04-24days-edge]: See [F9 in the appendix](./17-appendix-sources.md#f9-24days-caching-field-notes) for the 24days cache-busting and CDN field notes. These articles are historical community sources, not current v17 implementation sources.
